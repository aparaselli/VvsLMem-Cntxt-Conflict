...
def get_model_inputs(retrieved_context, user_query, entity_name=None, image=None, ctxt_image=None):
    ...
    # [REMOVED] 3. Suffix was here inside content_list. 
    # We do NOT add it to the user message anymore.

    # B. Generate Prompt using Template
    messages = [{"role": "user", "content": content_list}]
    
    # 3. Apply Template
    prompt_text = processor.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
    
    # 4. MANUALLY APPEND SUFFIX (Assistant Prefill)
    # This aligns the patching script with the eval script
    prompt_text += " Between A, B, C, and D, the answer is "
    
    # C. Handle Images (Same as before)
    active_images = []
    ...
    # Processor will now tokenize the full string including the prefilled suffix
...
def get_right_aligned_map(vis_ids, text_ids):
    """
    Returns [(vis_idx, text_idx)] mapping from the END of the sequence backwards.
...
def run_patching_experiment(vis_csv, text_csv, json_data_path, out_path):
    ...
    # --- FIX: Merge on ID AND Category/Query to avoid cross-products ---
    # Assuming 'Category' or 'Query' columns exist and differentiate the questions
    df_merged = pd.merge(
        df_vis, 
        df_text, 
        on=["ID", "Category", "Mis_Knowledge_Key"], # Use specific keys to align rows
        suffixes=("_vis", "_text")
    )
    
    # Validation: Ensure Ground Truths match (sanity check)
    # If this assertion fails, your rows are still not aligned correctly
    # assert (df_merged['Ground_Truth_vis'] == df_merged['Ground_Truth_text']).all()
    ...
    for idx, row in tqdm(diff_df.iterrows(), total=len(diff_df)):
        ...
        # 2. Vision Run (Cache Activations)
        ...
        # 5. Patching Loop (Sliding Window over ALL aligned tokens)
        WINDOW_SIZE = 3
        
        # Outer Loop: The Token Position (Right-Aligned)
        for v_idx, t_idx in alignment_map:
            token_pos_from_end = len(text_ids) - t_idx
            
            # Decode token string for heatmap labels
            token_id = text_ids[t_idx]
            token_str = processor.tokenizer.decode([token_id])
            
            # Inner Loop: Sliding Window of Layers
            for start_layer in range(len(transformer_layers) - WINDOW_SIZE + 1):
                
                active_hooks = []
                end_layer = start_layer + WINDOW_SIZE
                
                # Register hooks for the window
                for i in range(start_layer, end_layer):
                    src_vec = vision_cache[i][0, v_idx, :] # Use visual index
                    h = transformer_layers[i].register_forward_hook(
                        patcher.patch_hook(t_idx, src_vec) # Patch text index
                    )
                ...
                patch_results.append({
                    ...
                    "Token": token_str,
                    "Token_Pos_From_End": token_pos_from_end,
...
if __name__ == "__main__":
    ...
    OUT_CSV = "/users/aparasel/scratch/VvsLMem-Cntxt-Conflict/Scripts/Results/Patching_Query_SlidingWindow_Prob.csv"
