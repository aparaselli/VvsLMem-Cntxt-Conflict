...
def run_attribution(layer_type=LAYER_TYPE_DEFAULT, slice_idx=0, num_slices=10, bench_type="people", entity_modality="vision", mock_RAG=True):
    assert entity_modality in {"vision"}
    ...
    for _, row in tqdm(analysis_df.iterrows(), total=len(analysis_df)):
        ...
        if entity_modality == 'vision':
            ...
            inputs = make_inputs(
                ...
                bench_type="people",
                image=None,
                instance_name = ["Instance_text"],
                padding=True,          
                add_mcq_prefix=False,  
            )
        else:
            print("ERROR ENTITY MODALITY NOT SUPPORTED")
...
if __name__ == "__main__":
    ...
    run_attribution(layer_type=args.layer_type, slice_idx=args.slice_idx, num_slices=args.num_slices, bench_type = args.bench_type, entity_modality=args.entity_modality, mock_RAG = args.mock_RAG)
