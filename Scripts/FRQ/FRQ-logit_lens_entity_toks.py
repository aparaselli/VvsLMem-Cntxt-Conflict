...
OUT_CSV = "/users/aparasel/scratch/VvsLMem-Cntxt-Conflict/Scripts/Results/FRQ-entity_tok_LogitLens.csv"
OUT_CSV_logo = "/users/aparasel/scratch/VvsLMem-Cntxt-Conflict/Scripts/Results/FRQ-entity_tok_LogitLens_logo.csv"
...
def get_visual_positions_from_input_ids(input_ids_1d, tokenizer):
    """
    Returns a 1D LongTensor of positions corresponding to *visual patch tokens*
    (i.e., between <|vision_start|> and <|vision_end|>, excluding the markers).
    """
    ...
    # patch tokens are typically between start/end
    if e <= s + 1:
        return torch.empty(0, dtype=torch.long, device=input_ids_1d.device)
...
def run_logit_lens_fixed(dataset="celeb"):
    print("Loading Data...")
    if dataset == "celeb":
    ...
    def get_lens_hook(layer_idx, tid_gt, tid_cont, tid_output, tid_instance,vis_pos):
        def fn(module, input, output):
            # output[0]: [B, T, H]
            hs = output[0][0]  # [T, H]

            if vis_pos is None or vis_pos.numel() == 0:
            ...
            hs_vis = hs.index_select(0, vis_pos)  # [V, H]

            with torch.inference_mode():
                hs_vis = final_norm(hs_vis)                # [V, H]
                logits_vis = lm_head(hs_vis)               # [V, vocab]

                def max_logit_for(tid):
                    if tid is None:
                        return 0.0
                    return float(logits_vis[:, tid].max().item())

                # "max logit across visual tokens" for each target
                lg = max_logit_for(tid_gt)
                lc = max_logit_for(tid_cont)
                lo = max_logit_for(tid_output)
                le = max_logit_for(tid_instance)

                # Optional sanity-check: global best token *anywhere in vocab* across visual positions
                flat_max = logits_vis.max()
                flat_arg = int(logits_vis.view(-1).argmax().item())
                vocab_size = logits_vis.shape[-1]
                top_id = flat_arg % vocab_size
    ...
    for idx, row in tqdm(analysis_df.iterrows(), total=len(analysis_df)):
        ...
        instance_text = row['Instance_vis']
        ...
        instance_ids = processor.tokenizer.encode(" " + str(instance_text), add_special_tokens=False) 
        ...
        target_instance_id = instance_ids[0] if instance_ids else None
        # Register Hooks
        layer_data = {}
        hooks = []
        for i, layer in enumerate(transformer_layers):
            hooks.append(layer.register_forward_hook(
                get_lens_hook(i, target_gt_id, target_cont_id, target_output_id, target_instance_id, vis_pos)
            ))
        ...
        for i in range(len(transformer_layers)):
            d = layer_data.get(i, {
                "logit_GT": 0.0, "logit_Cont": 0.0, "logit_Output": 0.0, "logit_Entity": 0.0,
                "top_token_id": -1, "top_token": "", "top_logit": 0.0
            })
            results.append({
                "ID": row['ID'],
...
if __name__ == "__main__":
    ...
    run_logit_lens_fixed(args.dataset)