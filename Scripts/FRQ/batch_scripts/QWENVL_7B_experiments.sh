...
for modality in "${modalities[@]}"; do
  for bench in "${benches[@]}"; do
    for rag in "${rag_settings[@]}"; do
      ...
      OUT_PATH="${OUT_DIR}/Qwen_FRQ_${rag_suffix}_${modality^^}_Experiment_${bench^}_Results.csv"
...
python "$IDENTIFY_SCRIPT" \
  --bench_type people \
  --model_name "$MODEL" \
  --out_csv "${OUT_DIR}/Qwen_EntityID_People.csv"
...
python "$IDENTIFY_SCRIPT" \
  --bench_type logo \
  --model_name "$MODEL" \
  --out_csv "${OUT_DIR}/Qwen_EntityID_Logo.csv"
