...
def load_vlm(model_name: str):
    ...
    dtype = torch.float16  # RTX 6000 -> fp16 only
    ...
    if is_gemma:
        ...
        print("Gemma dtype:", dtype, "| bf16_supported:", bf16_ok)
        processor = AutoProcessor.from_pretrained(model_name, padding_side="left")
    else:
        processor = AutoProcessor.from_pretrained(model_name)
    if is_gemma:
        print("Gemma detected → enabling 2-GPU sharding")
        ...
        print("Qwen detected → single-GPU loading")
        ...
        model = ModelCls.from_pretrained(
            model_name,
            torch_dtype=dtype,
            device_map="auto",     # Qwen fits on one RTX 6000
            low_cpu_mem_usage=True,
        )
...
def rag_model_call(
    ...
    *,
    ...
    # print("logits nan?", torch.isnan(next_token_logits).any().item()) #DEBUGGGG
    # print("logits inf?", torch.isinf(next_token_logits).any().item())
    # print("logits max/min:", next_token_logits.max().item(), next_token_logits.min().item())
    # print("argmax id:", torch.argmax(next_token_logits).item(),
    #     "token:", processor.tokenizer.decode([torch.argmax(next_token_logits).item()]))
    if verbose:
    ...
    new_len = generated_ids.shape[-1] - input_len
    if verbose:
        print("new tokens:", new_len)
        print("first new ids:", generated_ids[0, input_len:input_len+10].tolist())
    if verbose:
...
def run_experiment(file_path, entity_modality='vision', mock_RAG=True,out_path=None, bench_type="people", ctxt_type="text", CoT=False,model=None,processor=None):

    with open(file_path, 'r') as file:
        data = json.load(file)
    if mock_RAG:
        mis_know_keys = ['mis_knowledge1', 'mis_knowledge2']
    else:
        mis_know_keys = ['mis_knowledge1']
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str2bool, 
        default=False, 
        help='Set to True to use Chain of Thought.'
    )
    ...
    parser.add_argument(
        ...
        help="HF model id (e.g., Qwen/Qwen2.5-VL-7B-Instruct or google/gemma-3-12b-it)"
    )
    ...
    model, processor, _dtype = load_vlm(args.model_name)
    run_experiment(file_path, entity_modality=args.entity_modality, mock_RAG=args.mock_RAG,out_path=args.out_path,bench_type=args.bench_type, ctxt_type=args.ctxt_type, CoT=args.CoT,model=model,processor=processor)